  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-40 p-2 sm:p-4 pointer-events-none flex justify-center">
      <div className="w-full max-w-6xl bg-zinc-950/90 backdrop-blur-2xl border border-white/15 rounded-2xl sm:rounded-full p-2 sm:px-5 sm:py-3 shadow-2xl shadow-black/90 pointer-events-auto flex flex-col md:flex-row items-center justify-between gap-3 transition-all duration-300">
          ...
          <div className="h-6 flex items-center overflow-hidden w-full md:w-auto">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                className="text-xs sm:text-sm font-medium text-zinc-200 truncate flex items-center gap-2"
              >
        ...
        {/* Right: Actions, Pagination, Currency, Language, Rules, Stats and Main CTA */}
        <div className="flex items-center justify-between md:justify-end gap-2 sm:gap-2.5 w-full md:w-auto shrink-0 flex-wrap sm:flex-nowrap">
          ...
          {totalPages > 1 && (
            <div className="flex items-center gap-1 bg-zinc-900/90 border border-white/10 p-1 rounded-full text-xs text-zinc-300">
              <button
                ...
                className="p-1 rounded-full hover:bg-white/10 disabled:opacity-30 disabled:hover:bg-transparent transition-colors cursor-pointer"
                title="Previous Page"
              >
                <ChevronLeft className="w-3.5 h-3.5" />
              </button>
              <span className="px-2 font-mono text-[11px] font-bold text-violet-300">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 rounded-full bg-zinc-900/90 hover:bg-zinc-800 border border-white/10 text-zinc-300 hover:text-white text-xs font-semibold transition-all cursor-pointer"
            title={t.ranking}
          >
          ...
          <button
            ...
            className="relative group overflow-hidden bg-gradient-to-r from-violet-600 via-purple-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 text-white font-bold text-xs sm:text-sm px-5 py-2.5 rounded-full shadow-lg shadow-violet-600/40 border border-violet-400/40 transition-all transform active:scale-95 flex items-center gap-2 cursor-pointer"
          >
